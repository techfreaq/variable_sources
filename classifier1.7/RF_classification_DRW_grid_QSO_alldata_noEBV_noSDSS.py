...
# galactic coordinates
   ...
   bad = (~np.isfinite(EBV_trainingset)) | (EBV_trainingset <= 0.0)
...
# training set: reset missing and unreliable WISE data 
   ...
   for i, (w12mag, w12err, w1mag,w2mag,w1err,w2err) in enumerate(zip(params_trainingset['W12'], params_trainingset['W12Err'], params_trainingset['w1mpro'], params_trainingset['w2mpro'], params_trainingset['w1sigmpro'], params_trainingset['w2sigmpro'])):
     if( (w1mag == 0.0) |  (w2mag == 0) |   (w1err <0) |   (w2err <0) | (w1err > 0.3) |   (w2err > 0.3)):   # (w1err > 0.1) |   (w2err > 0.1) |
...
# training set: reset missing and unreliable SDSS data           
   ...
   label[params_trainingset['SDSStype'] == 'QSO'] = 2 
   label[params_trainingset['SDSStype'] == 'RRLyrae'] = 1
...
#also, copy labels
   ...
   cPickle.dump(rf_qso, open('/a41233d1/hernitschek/_results_juelich/_results_classifier/_pickled_RFCs/RFC_QSO_reset-9999_noEBV_noSDSS_classweight.pickle', 'wb'), protocol=2)
   print("RF serialized")
...
if(make_trainingset==0):
  ...
  rf_qso = cPickle.load(open('/a41233d1/hernitschek/_results_juelich/_results_classifier/_pickled_RFCs/RFC_QSO_reset-9999_noEBV_noSDSS_classweight.pickle', 'rb')) #This function automatically determines whether the data stream was written in binary mode or not.
  print("RF loaded")
...
# in the end: write to a file that has corresponding name 
  ...
  # Eddie's dust map
      ...
      bad = (~np.isfinite(EBV_classify)) | (EBV_classify <= 0)
